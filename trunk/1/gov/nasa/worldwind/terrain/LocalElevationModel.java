...
/**
 * @author tag
 * @version $Id: LocalElevationModel.java 14278 2010-12-20 10:39:54Z tgaskins $
 */
public class LocalElevationModel extends AbstractElevationModel
{
    ...
    public void addElevations(ByteBuffer byteBuffer, Sector sector, int width, int height, AVList params)
        throws IOException
    {
        if (byteBuffer == null)
        {
        ...
        // Setup parameters to instruct BufferWrapper on how to interpret the ByteBuffer.
        AVList bufferParams = new AVListImpl();
        bufferParams.setValues(params.copy());

        if (!params.hasKey(AVKey.DATA_TYPE))
        {
            Object pixelType = params.hasKey(AVKey.PIXEL_TYPE) ? params.getValue(AVKey.PIXEL_TYPE) : AVKey.INT16;
            bufferParams.setValue(AVKey.DATA_TYPE, pixelType);
        }
        ...
        Double tileMissingDataFlag = AVListImpl.getDoubleValue(params, AVKey.MISSING_DATA_REPLACEMENT);
        if (tileMissingDataFlag == null)
            tileMissingDataFlag = this.getMissingDataSignal();

        // TODO: Check params for ELEVATION_MIN and ELEVATION_MAX and don't have the tile compute them if present.
