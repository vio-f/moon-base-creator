 ...
 *
 * @version $Id: SurfaceImage.java 14732 2011-02-16 19:02:32Z dcollins $
 */
...
//            }
    ...
    protected void initializeSourceTexture()
    {
        Object source = this.getImageSource();
        if (source instanceof BufferedImage)
        {
            // Create the texture immediately if the source is an in-memory BufferedImage.
            this.sourceTexture = new BasicWWTexture(source, true);
        }
        else if (!WWUtil.isEmpty(source.toString()))
        {
            // Otherwise, interpret the source as a path to the image source and request that the FileStore retrieve it.
            // The source may be a path to a local file, a resource on the class path, a URL string, a File object, or a
            // URL object. If the source is a remote image, this retrieves the image in another thread and does nothing
            // until it's loaded. We keep requesting the file here until it's successfully retrieved.
            URL sourceURL = WorldWind.getDataFileStore().requestFile(source.toString());
            if (sourceURL != null)
                this.sourceTexture = new BasicWWTexture(sourceURL, true);
        }
