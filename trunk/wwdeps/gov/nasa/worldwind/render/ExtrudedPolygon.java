 ...
 * boundaries or just the outer boundaries. Texture can not be applied to the cap. Standard lighting is optionally
 * applied. ExtrudedPolygon side faces and cap have independent attributes for both normal and highlighted drawing.
 * <p/>
 ...
 *
 * @author tag
 * @version $Id: ExtrudedPolygon.java 14744 2011-02-17 05:42:06Z dcollins $
 */
public class ExtrudedPolygon extends AbstractShape
{
    ...
    protected static class ShapeData extends AbstractShapeData implements Iterable<ExtrudedBoundaryInfo>
    {
        ...
        public ShapeData(DrawContext dc, ExtrudedPolygon shape)
        {
            ...
            // Copy the shape's side textures.
            if (shape.sideTextures != null && shape.sideTextures.size() > 0)
            {
    ...
    protected WWTexture capTexture; // TODO: test cap texturing
    /** The image source of the texture for this shape. */
    protected Object capImageSource;
    ...
    {
        ArrayList<WWTexture> sideTextures = new ArrayList<WWTexture>();

        for (Object source : imageSources)
        {
            if (source != null)
                sideTextures.add(new BasicWWTexture(source, true)); // TODO: implement proper protocol, as for cap image
            else
                sideTextures.add(null);
        }

        sideTextures.trimToSize();
     ...
     */
    public void setCapTextureImage(Object imageSource, float[] texCoords, int texCoordCount)
    {
        if (imageSource == null)
        {
    ...
    {
        Object imageSource = this.getCapImageSource();
    ...
    /**
     * Get the texture applied to this extruded polygon's cap. The texture is loaded on a background thread. This method
    ...
    protected boolean mustApplyTexture(DrawContext dc)
    {
        ...
        return this.mustApplySideTextures(dc);
    }

    /**
     * Returns the image for a specified face of the outer boundary.
     *
     * @param side the zero-origin position of the desired face.
     *
     * @return the image specified for the indicated face, or null if no outer boundary images are specified.
     */
    protected WWTexture getTexture(int side)
    {
        if (this.sideTextures == null || this.sideTextures.size() == 0)
            return null;

        return this.sideTextures.get(0) != null && this.sideTextures.get(0).size() > side
            ? this.sideTextures.get(0).get(side) : null;
    }
    ...
    protected boolean mustApplyLighting(DrawContext dc)
    {
        ...
        return !dc.isPickingMode() && this.getActiveSideAttributes().isEnableLighting();
    }
...
//            dc.drawNormals(1000, this.vertexBuffer, this.normalBuffer);
        ...
        boolean skipOuterBoundary)
    {
        ...
        if (this.isEnableSides())
        {
            ...
            if (this.mustApplySideTextures(dc))
    ...
    @Override
    protected void fillVBO(DrawContext dc) // TODO: implement AbstractShape's VBO protocol
    {
    }
    ...
    protected void createTessllationGeometry(DrawContext dc, ShapeData shapeData)
    {
        ...
        try
        {
            Vec4 normal = Vec4.normalVector3(shapeData.capVertexBuffer, 0);
            ...
            if (normal == null)
                normal = dc.getGlobe().computeSurfaceNormalAtPoint(shapeData.getReferencePoint());
    ...
    {
//        BoundarySet boundarySet = new BoundarySet(this);
     ...
     */
    public void moveTo(Position position) // TODO: test
    {
        if (position == null)
        {
        ...
        for (List<? extends LatLon> boundary : this.boundaries)
        {
            ...
            if (boundary.get(0) instanceof Position)
            {
