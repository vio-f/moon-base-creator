 ...
 * <p/>
 * When ths path's altitude mode is <code>RELATIVE_TO_GROUND</code>, the precision of terrain conformance is governed by
 * the path's follow-terrain and terrain-conformance properties. When the follow-terrain property is true, terrain
 * conformance adapts as the view moves relative to the path; the terrain-conformance property governs the precision of
 * conformance, and the number of intermediate positions computed varies. See {@link #setFollowTerrain(boolean)} and
 * {@link #setTerrainConformance(double)}. If the follow-terrain property is false, the view position is not considered
 * and the number of intermediate positions between specified positions is the constant value specified by the
 * num-subsegments property (see {@link #setNumSubsegments(int)}). The latter case may produce higher performance than
 * the former.
 * <p/>
 *
 * @author tag
 * @version $Id: Path.java 14750 2011-02-17 18:05:33Z tgaskins $
 */
public class Path extends AbstractShape
{
     ...
     *                  any positions in the set change, {@link #setPositions} must be called to inform this shape of
     *                  the change.
     *
    ...
    /**
     * Indicates whether this path is terrain following. This path is terrain-following when its altitude mode is {@link
     * WorldWind#CLAMP_TO_GROUND} or the follow-terrain flag has been explicitly set to <code>true</code>.
     *
    ...
    {
        return this.followTerrain || this.getAltitudeMode() == WorldWind.CLAMP_TO_GROUND;
    }

    /**
     * Specifies whether this path is terrain following. This path is terrain-following when its altitude mode is {@link
     * WorldWind#CLAMP_TO_GROUND} or the follow-terrain flag has been explicitly set to <code>true</code>.
     *
