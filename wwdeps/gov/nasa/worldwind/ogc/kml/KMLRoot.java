 ...
 * <p/>
 * KMLRoot also serves as a factory for creating description balloons for KML features. The type of balloon created can
 * be changed using the {@link AVKey#KML_USE_BROWSER_BALLOON} {@link Configuration} setting. If this setting is true,
 * KMLRoot will create balloons of type {@link AbstractBrowserBalloon}. Otherwise it will create {@link
 * AbstractAnnotationBalloon}.
 *
 * @author tag
 * @version $Id: KMLRoot.java 14657 2011-02-10 00:27:29Z dcollins $
 */
public class KMLRoot extends KMLAbstractObject implements KMLRenderable
{
     ...
     * local document, including the current document, and {@code linkRef} is the id of the desired element.
     * <p/>
     * If {@code linkBase} refers to a support file in a KMZ, the return value is file path to the extracted file. If
     * {@code linkBase} refers to a KML or KMZ file and {@code linkRef} is non-null, the return value is the element
     * identified by {@code linkRef}. Otherwise, if {@code linkRef} is null, the return value is a parsed {@link
     * KMLRoot} for the KML file identified by {@code linkBase}.
     *
    ...
    public Object resolveRemoteReference(String linkBase, String linkRef)
    {
        try
        {
            ...
            if (contentType != null)
            {
                ...
                {
                    KMLRoot refRoot = new KMLRoot(url, contentType);
                    refRoot.parse();
                    WorldWind.getSessionCache().put(linkBase, refRoot);
                    if (linkRef != null)
                        return refRoot.getItemByID(linkRef);
     ...
     * @param balloonAttachmentMode The balloon mode, either {@link AVKey#GLOBE} or {@link AVKey#SCREEN}.
     *
    ...
    public Balloon createBalloon(KMLAbstractFeature feature, KMLBalloonStyle style, String balloonAttachmentMode)
    {
        ...
        // Read the configuration setting that determines what type of balloon to create.
        Boolean useBrowserBalloon = Configuration.getBooleanValue(AVKey.KML_USE_BROWSER_BALLOON, false);

        AbstractKMLBalloon kmlBalloon;
        if (AVKey.GLOBE.equals(balloonAttachmentMode))
        {
            GlobeBalloon balloon;
            if (useBrowserBalloon)
